...
#[serde(default)]
pub struct XrayFFTF {
    /// Sampling and window domain. Default: `FFTGrid::Larch` in this ndarray compatibility backend.
...
impl XrayFFTF {
    ...
    /// Resolve automatic settings from at least two k samples without calculating.
    ///
    ...
    /// Validate input/settings, resolve automatic values and allocate prepared data.
    /// Returns `(chi * k.powi(kweight), window)` before multiplying the window.
    /// The Larch default resamples from k=0; Input preserves the supplied samples.
    /// No Fourier coefficients are calculated by this preparation step.
    pub fn xftf_prep(
        &mut self,
    ...
    /// Compute the weighted, windowed forward transform and replace cached results.
    ///
    /// Requires matched finite arrays with at least two increasing k samples. The
    /// compatibility default is Larch resampling from zero. Invalid settings or
    /// an FFT too short for that extended grid return `FFTError`.
    pub fn xftf(
        &mut self,
        k: ArrayBase<ViewRepr<&f64>, Ix1>,
        chi: ArrayBase<ViewRepr<&f64>, Ix1>,
...
/// Transform already weighted/windowed ndarray input using `kstep / sqrt(pi)`.
/// Uses an unnormalized negative-exponent FFT and zero-pads or truncates to
/// `nfft`. This low-level helper does not validate parameters.
...
/// Restore conjugate symmetry and return a real ndarray inverse signal.
/// The unnormalized inverse is multiplied by `sqrt(pi)/(kstep*nfft)`;
/// this low-level helper does not validate parameters.
pub fn xftr_fast(
    chir: &DynRealDft<f64>,
    nfft: usize,
    kstep: f64,
