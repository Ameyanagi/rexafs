...
def close_test_gui(root, target, output):
    ...
    if os.name == "nt":
            ...
            lambda: (
                subprocess.check_output(
                    [
                        ...
                        "-NonInteractive",
                        "-Command",
                        f"(Get-Process -Id {pid} -ErrorAction Stop).MainWindowHandle",
                    ],
                    encoding="utf-8",
                ).strip()
                != "0"
            ),
            "reopened Windows window",
            60,
        )
        subprocess.run(
            ["taskkill", "/PID", str(pid), "/T", "/F"], check=True, capture_output=True
        )
    else:
        ...
        time.sleep(2)
        subprocess.run(
            ["import", "-window", window_id(), str(output / "reopened.png")], check=True
        )
...
def exercise(source, target, project, output, setup=None, failure=False):
    ...
    plan = {
        ...
        "previous_hash": digest(target / name),
        "manifest_hash": digest(incoming / "update-manifest.json"),
    ...
    with (root / "install.log").open("w") as log:
        helper = subprocess.Popen(
            [str(helper_dir / name), "--finish-update", str(root)],
            cwd=root,
        ...
        try:
            wait_until(
                lambda: (root / "ready").is_file() or helper.poll() is not None,
                "helper readiness",
            )
            assert helper.poll() is None, (root / "install.log").read_text()
            assert not (root / "previous").exists(), (
                "App changed while the parent was running"
            )
            parent.stdin.close()
            ...
            result = (
                ...
                else (root / "install.log").read_text()
            )
            ...
            if failure:
                ...
                assert after == before, (
                    "Failed update changed the original installation"
                )
            else:
                assert (root / "previous" / name).is_file()
                for entry in json.loads((source / "update-manifest.json").read_text())[
                    "files"
                ]:
                    assert digest(target / entry["path"]) == entry["sha256"], entry[
                        "path"
                    ]
                ...
                json.dumps(
                    {
                        ...
                        "installer": bool(setup),
                        "expected_failure": failure,
                        "checks": [
                            "wait-for-parent",
                            "recovery-launch",
                            "preserve-user-project",
                            "restore-original-files"
                            if failure
                            else "install-verified-payload",
                        ],
                        "result": result,
                    },
                    indent=2,
            ...
            if helper.poll() is None:
                ...
                raise RuntimeError(
                    f"Updater still running; retain transaction for diagnosis: {root}"
                )
    shutil.rmtree(root)
...
def main():
    ...
    project = (
        ...
        / "crates/rexafs-gui/tests/fixtures/projects/rexafs-0.2.8-embedded.rxs"
    )
    if args.installed:
        ...
        if not target.is_relative_to(runner_temp) or not args.setup:
            parser.error(
                "Installed qualification must use the disposable installer-test directory and matching setup"
            )
        for failure in [False, True]:
            exercise(
                ...
                target,
                project,
                args.output / ("installer-rollback" if failure else "installer-update"),
                args.setup.resolve(strict=True),
                failure,
            )
    else:
        ...
        for failure in [False, True]:
            target = Path(directory) / (
                "failure 日本語" if failure else "success 日本語"
            )
            ...
            exercise(
                ...
                target,
                project,
                args.output / ("portable-rejection" if failure else "portable-update"),
                failure=failure,
            )
