...
const source = (path, channel) => channel === 'stable'
...
for (const channel of ['stable', 'next']) {
  ...
  for (const path of ['js-rexafs/types.d.ts', 'js-rexafs/index.d.ts', 'js-rexafs/node.d.ts']) {
    ...
    const currentFile = ts.createSourceFile(path, source(path, 'next'), ts.ScriptTarget.Latest, true);
    ...
    for (const node of file.statements) {
      ...
      const lines = [
        ...
        `**${channel === 'stable' ? 'Stable ' + tag.slice(1) : 'Next API · unreleased'}.** ${channel === 'stable'
          ...
          : `These signatures describe the source checkout, not npm rexafs@${tag.slice(1)}.`}`,
